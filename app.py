...
def chat_api():
    ...
    Returns:
        JSON response with bot message
    """
    try:
        ...
        session_id = data.get('session_id', f'web_session_{uuid.uuid4().hex[:8]}')
        
        if not user_message:
            return jsonify({'error': 'Message cannot be empty'}), 400
        
        logger.info(f"Web chat message from {session_id}: {user_message}")
        
        # Handle FX commands first
        fx_response = handle_fx_commands(user_message)
        if fx_response:
            return jsonify({
                'message': fx_response,
        ...
        if openai_client:
            try:
                ...
                return jsonify({
                    'message': "I'm experiencing technical difficulties. Please try asking about FX rates or currency exchange.",
                    'session_id': session_id,
                    'timestamp': datetime.now().isoformat()
                })
